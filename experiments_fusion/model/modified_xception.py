...
class Xception(nn.Module):
    ...
    def __init__(self, num_classes=1000, in_chans=3, drop_rate=0., global_pool='avg', modified = False, pre_logits = False, OC=False):
        """ Constructor
        Args:
    ...
    def forward_head(self, x, pre_logits: bool = False):
        ...
        return x, self.fc(x) if self.pre_logits else self.fc(x)

    def forward(self, x):
        x = self.forward_features(x)
        if self.OC == True:
            return self.global_pool(x)
        elif self.pre_logits == True:
            latent_vector, x = self.forward_head(x)
            return latent_vector, x
        else:
            x = self.forward_head(x)
            return x

def load_model(num_classes=2, modified = False, pre_logits = False, transfer_weight_path=None, OC=False):
    model = Xception(num_classes = num_classes, modified = modified, pre_logits = pre_logits, OC=OC)
    if transfer_weight_path:
        state_dict = torch.load(transfer_weight_path)
        
        if 'model_dict' in state_dict.keys():
            weights = state_dict['model_dict']
        else:
            weights = state_dict

        if 'pretrained_model.conv1.weight' in weights.keys():
            new_weights = {key.replace('pretrained_model.', ''): value for key, value in weights.items()}
            model.load_state_dict(new_weights, strict=False)
    
        else:
            weights = {key: value for key, value in weights.items() if not key.startswith('fc')} # without fc
